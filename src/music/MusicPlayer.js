...
class MusicPlayer {
        ...
        try {
            ...
            if (track.platform === 'youtube' || track.platform === 'spotify' || track.platform === 'soundcloud') {
                ...
                try {
                    ...
                    // YouTube đôi khi chặn tải đầy đủ (n-challenge / SABR). Fallback: fetch luồng stream rồi transcode bằng FFmpeg.
                    console.warn(`⚠️ yt-dlp download failed for ${track.title}, falling back to stream transcode: ${downloadError.message}`);
                    await this.transcodeToFile(streamUrl, filepath, streamInfo);
                }
            } else {
    ...
    /**
     * Fetches an audio stream URL and transcodes it to an opus file with FFmpeg.
     * Used for DirectLink tracks and as a fallback when yt-dlp full downloads are blocked.
     */
    async transcodeToFile(url, filepath, streamInfo) {
    ...
    async handleTrackEnd(reason = 'idle') {
        ...
        try {
            ...
            // Bot ở lại kênh voice sau khi hết nhạc (không tự leave).
            // Chỉ rời kênh khi: user dùng lệnh leave/musicstop, bị ngắt kết nối,
            // hoặc không còn ai trong kênh (inactivity timer trong voiceStateUpdate).
        } finally {
            this.isTransitioning = false;
            this.skipRequested = false;
            this.stopRequested = false;
